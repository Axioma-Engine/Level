 ...
 * @brief GCC/Clang bit-operation implementation.
 *
 * @details
 * This backend provides bit-manipulation primitives using GCC/Clang compiler
 * builtins where available, with portable software fallbacks for functions
 * that may not have builtin support.
 *
 ...
 */
#ifndef AXM_LVL_GCC_BITOPERATIONS_H
#define AXM_LVL_GCC_BITOPERATIONS_H
...
#if AXM_HAS_BUILTIN(__builtin_ctz)
        return value ? __builtin_ctz(value) : sizeof(u32) * 8;
#else
...
#if AXM_HAS_BUILTIN(__builtin_ctzll)
        return value ? __builtin_ctzll(value) : sizeof(u64) * 8;
#else
...
#if AXM_HAS_BUILTIN(__builtin_clz)
        return value ? __builtin_clz(value) : sizeof(u32) * 8;
#else
        if (value == 0) return sizeof(u32) * 8;
...
#if AXM_HAS_BUILTIN(__builtin_clzll)
        return value ? __builtin_clzll(value) : sizeof(u64) * 8;
#else
        if (value == 0) return sizeof(u64) * 8;
...
#if AXM_HAS_BUILTIN(__builtin_popcount)
...
#if AXM_HAS_BUILTIN(__builtin_popcountll)
...
#if AXM_HAS_BUILTIN(__builtin_bswap16)
...
#if AXM_HAS_BUILTIN(__builtin_bswap32)
...
#if AXM_HAS_BUILTIN(__builtin_bswap64)
